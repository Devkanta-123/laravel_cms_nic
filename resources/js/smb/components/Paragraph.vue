<template>
    <div class=" container hp_container">
      <div class="content hp_content">
        ...
        <div v-else-if="currentLanguage === 'Hindi'">
            <div v-html="hindi_paragraph"></div> 
        </div>
  ...
  const heading = ref('');
  const hindi_heading = ref(''); 
  const khasi_heading = ref(''); 
  const paragraph = ref(''); 
  const hindi_paragraph = ref(''); 
  const khasi_paragraph = ref(''); 
  const menuItems = ref([]);
  ...
  const fetchPara = async () => {
    try {
      ...
      heading.value = response.data.title;
      paragraph.value= response.data.content; 
      
      hindi_heading.value = response.data.hindi_title;
      hindi_paragraph.value= response.data.hindi_content;
